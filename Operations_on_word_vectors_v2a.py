...
def neutralize(word, g, word_to_vec_map):
    ...
    e_biascomponent = ((np.dot(e, g))/(np.sqrt(np.dot(g, g.T)))**2)*g
...
def equalize(pair, bias_axis, word_to_vec_map):
    ...
    mu_B = (np.dot(mu, bias_axis)/ (np.linalg.norm(bias_axis, 2)**2)) * bias_axis                                    
    mu_orth = mu - mu_B

    # Step 4: Use equations (7) and (8) to compute e_w1B and e_w2B (≈2 lines)
    e_w1B = (np.dot(e_w1, bias_axis)/ (np.linalg.norm(bias_axis, 2)**2)) * bias_axis
    e_w2B = (np.dot(e_w2, bias_axis)/ (np.linalg.norm(bias_axis, 2)**2)) * bias_axis
        
    # Step 5: Adjust the Bias part of e_w1B and e_w2B using the formulas (9) and (10) given above (≈2 lines)
    corrected_e_w1B = np.sqrt(np.abs(1 - np.linalg.norm(mu_orth,2)**2)) * ((e_w1B-mu_B) / np.linalg.norm((e_w1-mu_orth)-mu_B, 2))
    corrected_e_w2B = np.sqrt(np.abs(1 - np.linalg.norm(mu_orth,2)**2)) * ((e_w2B-mu_B) / np.linalg.norm((e_w2-mu_orth)-mu_B, 2))
